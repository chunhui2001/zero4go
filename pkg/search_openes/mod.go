...
type ESConf struct {
	Enable      bool   `mapstructure:"ES_OPEN_ENABLE"`
	Servers     string `mapstructure:"ES_OPEN_SERVERS"`
	DslLocation string `mapstructure:"ES_OPEN_DSL_LOCATION"`
}

var Settings = &ESConf{
	Enable:      true,
	Servers:     "http://localhost:9200",
	DslLocation: "./META-INF/es_open_dsl",
}
...
func Init() {
	...
	if err != nil {
		Log.Errorf("OpenSearch-init-Failed: Server=%s, Error=%s", Settings.Servers, err.Error())
...
func (c *EsClient) Ping() {
	// 构造 HTTP 请求
	req, err := http.NewRequestWithContext(context.Background(), "GET", "http://localhost:9200/", nil)
	...
	if err := json.NewDecoder(res.Body).Decode(&r); err != nil {
		...
		Log.Info(fmt.Sprintf(
			"OpenSearch-Init-Succeed: Servers=%s, ClusterName=%s, ServerVersion=%s, LuceneVersion=%s, ClientDriverVersion=%s",
			Settings.Servers, clusterName, serverVersion, luceneVersion, opensearch.Version),
		)
	}
}
...
func (c *EsClient) CatIndices() ([]map[string]interface{}, error) {
	...
	if err != nil {
		Log.Errorf("OpenSearch-CatIndices-Failed: Error=%s", err.Error())
...
func (c *EsClient) SaveOrUpdate(indexName string, id string, dataMap map[string]interface{}) (string, error) {
	...
	if err != nil {
		Log.Errorf("OpenSearch-SaveOrUpdate-Failed: Error=%s", err.Error())
	...
	if resMap["error"] != nil {
		Log.Errorf("OpenSearch-SaveOrUpdate-Failed: Error=%s", utils.ToJsonString(resMap["error"]))
...
func (c *EsClient) Bulk(indexName string, dataMap *[]map[string]interface{}) (bool, error) {
	...
	if err != nil {
		Log.Errorf("OpenSearch-Bulk-Failed: Error=%s", err.Error())
		return false, err
	}
...
func (c *EsClient) getBulkIndexerItem(item *map[string]interface{}, countSuccessful *uint64) esutil.BulkIndexerItem {
	...
	return esutil.BulkIndexerItem{
		...
		OnFailure: func(ctx context.Context, item esutil.BulkIndexerItem, res esutil.BulkIndexerResponseItem, err error) {
			if err != nil {
				Log.Errorf("OpenSearch-getBulkIndexerItem-Failed: Error=%s", err.Error())
			} else {
				Log.Errorf("OpenSearch-getBulkIndexerItem-Failed: ErrorType=%s, Error=%s", res.Error.Type, res.Error.Reason)
			}
		},
	}
}
